...
#define MZ_SOLDIER_PLACEHOLDER 0

#define SOLDIER_BLASTER_CLIP 20
#define SOLDIER_MG_CLIP 30
#define SOLDIER_HB_CLIP 20
#define SOLDIER_SG_CLIP 1
#define SOLDIER_RG_CLIP 1
#define SOLDIER_GRENADE_CLIP 1

#define SOLDIER_ATTACK1_SKIPATTACKFRAME 6
#define SOLDIER_ATTACK2_SKIPATTACKFRAME 8
#define SOLDIER_ATTACK3_SKIPATTACKFRAME 7
#define SOLDIER_ATTACK4_SKIPATTACKFRAME 4
#define SOLDIER_ATTACK6_SKIPATTACKFRAME 9
#define SOLDIER_ATTACK7_SKIPATTACKFRAME 8
...
void soldier_cock(edict_t* self)
{
	...
	if (mg_guard(self))
	{
		if (self->monsterinfo.aiflags & AI_SHOOTGRENADE && self->monsterinfo.clip2 == 0)
			...
			self->monsterinfo.clip = SOLDIER_MG_CLIP;
	}
	if (rg_guard(self))
	{
		self->monsterinfo.clip = SOLDIER_RG_CLIP;
	}
	if (hb_guard(self))
	{
		self->monsterinfo.clip = SOLDIER_HB_CLIP;
	}
...
void soldier_stand3_skip(edict_t* self)
{
	if (self->s.skinnum < 2 || random() < 0.95) // this looks and sounds silly when they pump the shotgun (weapon) all the time
...
void update_move_dir(edict_t* self)
{
	...
	if (self->monsterinfo.move_dir == MOVE_LEFT)
	{
		...
		else if ((self->s.frame >= FRAME_run_r02 && self->s.frame <= FRAME_run_r08) ||
			(self->s.frame >= FRAME_run02 && self->s.frame <= FRAME_run08))
		{
			self->monsterinfo.currentmove = &soldier_move_run_l;
			update_move_frame(self);
		}
	...
	if (self->monsterinfo.move_dir == MOVE_RIGHT)
	{
		if (self->s.frame == FRAME_run_l01 || self->s.frame == FRAME_run01)
		{
			...
			if ((self->s.frame >= FRAME_run_l02 && self->s.frame <= FRAME_run_l08) ||
				(self->s.frame >= FRAME_run02 && self->s.frame <= FRAME_run08))
			{
				self->monsterinfo.currentmove = &soldier_move_run_r;
				update_move_frame(self);
	...
	if (self->monsterinfo.move_dir == MOVE_STRAIGHT)
	{
		if (self->s.frame == FRAME_run_r01 || self->s.frame == FRAME_run_l01)
		{
			...
			(self->s.frame >= FRAME_run_l02 && self->s.frame <= FRAME_run_l08))
		{
			self->monsterinfo.currentmove = &soldier_move_run;
			update_move_frame(self);
		}
...
static int railgun_flash[] = { MZ2_SOLDIER_RAILGUN_1, MZ2_SOLDIER_RAILGUN_2, MZ2_SOLDIER_RAILGUN_3, MZ2_SOLDIER_RAILGUN_4, MZ2_SOLDIER_RAILGUN_5, MZ2_SOLDIER_RAILGUN_6, MZ2_SOLDIER_RAILGUN_7, MZ2_SOLDIER_RAILGUN_8 };
static int machinegun_flash[] = { MZ2_SOLDIER_MACHINEGUN_1, MZ2_SOLDIER_MACHINEGUN_2, MZ2_SOLDIER_MACHINEGUN_3, MZ2_SOLDIER_MACHINEGUN_4, MZ2_SOLDIER_MACHINEGUN_5, MZ2_SOLDIER_MACHINEGUN_6, MZ2_SOLDIER_MACHINEGUN_7, MZ2_SOLDIER_MACHINEGUN_8, MZ_SOLDIER_PLACEHOLDER, MZ2_SOLDIER_MACHINEGUN_9, MZ2_SOLDIER_MACHINEGUN_10, MZ2_SOLDIER_MACHINEGUN_11, MZ2_SOLDIER_MACHINEGUN_12, MZ2_SOLDIER_MACHINEGUN_13, MZ2_SOLDIER_MACHINEGUN_14 };
static int hblaster_flash[] = { MZ2_SOLDIER_HBLASTER_1, MZ2_SOLDIER_HBLASTER_2, MZ2_SOLDIER_HBLASTER_3, MZ2_SOLDIER_HBLASTER_4, MZ2_SOLDIER_HBLASTER_5, MZ2_SOLDIER_HBLASTER_6, MZ2_SOLDIER_HBLASTER_7, MZ2_SOLDIER_HBLASTER_8, MZ_SOLDIER_PLACEHOLDER, MZ2_SOLDIER_HBLASTER_9, MZ2_SOLDIER_HBLASTER_10, MZ2_SOLDIER_HBLASTER_11, MZ2_SOLDIER_HBLASTER_12, MZ2_SOLDIER_HBLASTER_13, MZ2_SOLDIER_HBLASTER_14 };
...
void soldier_attack1_refire1(edict_t* self)
{
	...
	if (soldier_isneedingreload(self) || range(self, self->enemy) >= RANGE_MID)//if needing reload just progress further
	{
		//soldier_gotoreload(self);
	...
	{
		self->monsterinfo.nextframe = FRAME_attak109;
		return;
...
void soldier_attack1_refire2(edict_t* self)
{
	...
	if ((((skill->value == 3) && (random() < 0.5)) ||
		...
		(self->monsterinfo.aggression + random() < 1.75f && skill->value > 3)))
...
{
	{ai_charge, 0,  NULL},
	{ai_charge, 0,  soldier_gotoreload},
...
void soldier_fire3(edict_t* self)
{
	...
	soldier_fire(self, 2);
	if ((mg_guard(self) || hb_guard(self)) && self->monsterinfo.burstnum < 3 && self->monsterinfo.aiflags & AI_HOLD_FRAME)
		return;
...
void soldier_attack3_refire(edict_t* self)
{
	if (self->groundentity)
	{
		if (self->monsterinfo.aiflags & AI_JUMPDODGE ||
			...
			((visible(self->enemy, self) && (!infront_aiming(self->enemy, self)) && random() > 0.95) ||
				infront_aiming(self->enemy, self)))
		{
			soldier_jump(self);
			return;
		}
	...
	if ((((skill->value == 3) && (random() < 0.5)) ||
		range(self, self->enemy) == RANGE_MELEE || ((random() * 4) - self->monsterinfo.aggression > 1 && skill->value > 3)))
	{
...
mframe_t soldier_frames_attack6[] =
{
	{ai_charge, 10, NULL},
	{ai_charge,  4, NULL},
	{ai_charge, 12, soldier_gotoreload},
	...
	{ai_charge, 11, NULL},
	{ai_charge,  8, soldier_fire8_cock},
...
void soldier_fire10(edict_t* self)
{
	soldier_fire(self, 10);
}
void soldier_fire11(edict_t* self)
{
	soldier_fire(self, 11);
}
void soldier_fire12(edict_t* self)
{
	soldier_fire(self, 12);
}
void soldier_fire13(edict_t* self)
{
	soldier_fire(self, 13);
}
void soldier_fire14(edict_t* self)
{
	soldier_fire(self, 14);
}
void soldier_attack7_refire(edict_t* self)
{
	...
	if (range(self, self->enemy) < RANGE_NEAR || VectorCompare(self->s.origin, self->old_origin2))
		return;
	soldier_fire(self, 13);
	self->monsterinfo.nextframe = FRAME_runs_m03;
}
void soldier_fire9_cock(edict_t* self)
{
	if (self->enemy->health <= 0)
		return;
	soldier_cock(self);
...
{
	{ai_charge, 10, NULL},
	{ai_charge,  4, soldier_gotoreload},
	{ai_charge, 12, soldier_fire9},
	{ai_charge, 11, soldier_fire10},
	{ai_charge, 13, soldier_fire11},
	{ai_charge, 18, soldier_fire12},
	{ai_charge, 15, soldier_fire13},
	{ai_charge, 14, soldier_attack7_refire},
	{ai_charge, 11, soldier_fire9_cock},
	{ai_charge,  8, NULL},
	{ai_charge, 11, NULL},
	{ai_charge, 12, NULL},
	{ai_charge, 12, NULL},
};
mmove_t soldier_move_attack7 = { FRAME_runs_m01, FRAME_runs_m13, soldier_frames_attack7, soldier_run };
...
void soldier_check_grenade(edict_t* self)
{
	if (rg_guard(self))
		return;
	if (self->monsterinfo.aiflags & AI_SHOOTGRENADE || (self->s.skinnum <= 1 || self->s.skinnum > 5))
		return;
	vec3_t dist;
...
check_fire:
	{
		if (bl_guard(self) || sg_guard(self) || rg_guard(self) || self->monsterinfo.aiflags & AI_SHOOTGRENADE) //there was a check to not put machine gun guard into run attack, 99 is a placeholder for letting it through
		{
			soldier_check_grenade(self);

			if (bl_guard(self) && skill->value / 15 > mt_ldrand())
			{
				start_charge(self);
			}
			...
			if (!rg_guard(self) && (range(self, self->enemy) > RANGE_NEAR) && check_run(self))
			{
				self->monsterinfo.currentmove = &soldier_move_attack6;
				return;
			}

			if (!mg_guard(self) && !hb_guard(self))
			{
				if (!rg_guard(self) && (range(self, self->enemy) > RANGE_MELEE) && check_run(self) && (random() + self->monsterinfo.aggression > 1.5f))
				{
					self->monsterinfo.currentmove = &soldier_move_attack6;
					return;
				}
			...
			if (mg_guard(self) || hb_guard(self))
				goto check_fire;
			if ((range(self, self->enemy) > RANGE_MELEE) && check_run(self))
			{
				self->monsterinfo.currentmove = &soldier_move_attack6;
				return;
			}
		...
		else if (mg_guard(self) || hb_guard(self))
		{
			soldier_check_grenade(self);
...
void soldier_sight(edict_t* self, edict_t* other)
{
	...
	if ((skill->value > 0) && (range(self, self->enemy) > RANGE_MELEE))
	{
...
void soldier_gotoreload(edict_t* self)
{
	if (!soldier_isneedingreload(self))
		return;
	if (self->monsterinfo.currentmove == &soldier_move_attack1)
	{
		self->monsterinfo.nextframe = FRAME_attak101 + SOLDIER_ATTACK1_SKIPATTACKFRAME - 1;
	}
	else if (self->monsterinfo.currentmove == &soldier_move_attack2)
	{
		self->monsterinfo.nextframe = FRAME_attak201 + SOLDIER_ATTACK2_SKIPATTACKFRAME - 1;
	}
	else if (self->monsterinfo.currentmove == &soldier_move_attack3)
	{
		self->monsterinfo.nextframe = FRAME_attak301 + SOLDIER_ATTACK3_SKIPATTACKFRAME - 1;
	}
	else if (self->monsterinfo.currentmove == &soldier_move_attack4)
	{
		self->monsterinfo.nextframe = FRAME_attak401 + SOLDIER_ATTACK4_SKIPATTACKFRAME - 1;
	}
	else if (self->monsterinfo.currentmove == &soldier_move_attack6)
	{
		self->monsterinfo.nextframe = FRAME_runs01 + SOLDIER_ATTACK6_SKIPATTACKFRAME - 1;
	}
	else if (self->monsterinfo.currentmove == &soldier_move_attack7)
	{
		self->monsterinfo.nextframe = FRAME_runs_m01 + SOLDIER_ATTACK7_SKIPATTACKFRAME - 1;
	}
	else
	{
		if (rand() & 1)
		...
		{
			self->monsterinfo.currentmove = &soldier_move_attack2;
			self->monsterinfo.nextframe = FRAME_attak201;
		}
	}
}
void soldier_fire(edict_t* self, int flash_number)
{
	...
	else
	{
		...
		else if (sg_guard(self))
			flash_index = shotgun_flash[flash_number];
		else if (rg_guard(self))
			flash_index = railgun_flash[flash_number];
		else
		{
			if (hb_guard(self))
				flash_index = hblaster_flash[flash_number];
			else
	...
	else
	{
		if (flash_number == 5 || flash_number == 6)
		{
			...
			if (visible(self, self->enemy) && FacingIdeal(self))
			{
				...
				if (self->enemy->health > 0)
					end[2] += self->enemy->viewheight / 2.0f;
		...
		if (!shoot_infront && flash_number != 8)
		{
			...
			else if (bl_guard(self) || hb_guard(self))
	...
	else if (rg_guard(self))
	{
		...
		if (skill->value > 3)
			damage *= skill->value / 2;
		self->monsterinfo.clip -= 1;
		monster_fire_railgun(self, start, aim, damage, 100, flash_index);
	}
	else if ((mg_guard(self) || hb_guard(self)) && self->s.frame >= FRAME_runs_m01 && self->s.frame <= FRAME_runs_m13) //special run & shoot anim for machinegun guard
	{
		if (mg_guard(self))
		{
			damage = 2;
...
void check_fire(edict_t* self)
{
	...
	if (((!visible(self, self->enemy) && !infront(self, self->enemy)) ||
		...
		((self->count > 5 + (self->monsterinfo.aggression * 5) && self->s.skinnum <= 3) ||
...
{
	if (self->s.skinnum == 0 || self->s.skinnum == 1)
		return true;
	else
		return false;
}
...
{
	if (self->s.skinnum == 4 || self->s.skinnum == 5)
		return true;
	else
		return false;
}
...
{
	if (self->s.skinnum == 8 || self->s.skinnum == 9)
		return true;
	else
		return false;
}
