	...
	if (deathmatch->value || coop->value)
	{
		...
		if (attacker && attacker->client)
		{
			...
			case MOD_MACHINEGUN_EMP:
				message = "has it's circuits fried by";
				break;
			case MOD_TELEFRAG:
	...
	{	// normal death
		if (!self->deadflag)
		{
			...
			case 3:
				...
				self->client->anim_end = FRAME_death417;
				//gi.bprintf(PRINT_HIGH, "player_die: ADDITIONAL DEATH ANIM!!!");
				break;
			}
...
void CopyToBodyQue(edict_t* ent)
{
	...
	body->count = ent->client->anim_end; //saving here animation stop in case we respawn quicker
	...
	if (ent->svflags & SVF_NOBODYGIBS)
		body->die = head_diepain;
	else
		body->die = body_die;
	body->takedamage = DAMAGE_YES;
	body->think = bodyque_think;
...
void ClientUserinfoChanged(edict_t* ent, char* userinfo)
{
	...
	{
		Q_strncpyz(userinfo, sizeof userinfo, "\\name\\badinfo\\skin\\male/grunt");
	}
	...
	if (!strcmp(s, "male"))
	{
		ent->svflags |= SVF_MALESKIN;
		//gi.bprintf(PRINT_HIGH, "ClientUserinfoChanged: AKNOWLEDGED THAT SKIN IS MALE!");
	}
	else
		ent->svflags &= ~SVF_MALESKIN;
...
void ClientThink(edict_t* ent, usercmd_t* ucmd)
{
	...
	if (level.time > 1) // we cannot print saved ucmd when it was not actually saved from a valid one, it crashes, so let's wait
	{
		if (level.time > 2)
		{
			client->olducmd = client->ucmd;
			//client->oldforwardmove = client->forwardmove;
			//client->oldsidemove = client->sidemove;
		}

		client->ucmd = ucmd; //save the ucmd
		//client->forwardmove = ucmd->forwardmove;
		//client->sidemove = ucmd->sidemove;
		//if (level.time > 2)
		//gi.bprintf(PRINT_HIGH, "DEBUG: forwardmove = %i, old forwardmove %i, sidemove = %i, old sidemove = %i\n", (int)client->forwardmove, (int)client->oldforwardmove, (int)client->sidemove, (int)client->oldsidemove);

	}
	//gi.bprintf(PRINT_HIGH, "ClientThink: frame = %i, anim priority = %i\n", ent->s.frame, client->anim_priority);
	//if(!deathmatch->value)
	//	ClientTrace(ent);

	//gi.bprintf(PRINT_HIGH, "origin = %s, old origin = %s, client think at %f\n", vtos(ent->s.origin), vtos(ent->s.old_origin), level.time);
	//-scanner(ent);
	//gi.bprintf(PRINT_HIGH, "DEBUG:  buttons = %i\n", client->buttons);

	if (level.intermissiontime)
	{
		client->buttons = ucmd->buttons;
		//gi.bprintf(PRINT_HIGH, "DEBUG: INTERMISSION: level.time = %f, level.intermissiontime = %f, buttons = %i\n", level.time, level.intermissiontime, client->buttons);
		client->ps.pmove.pm_type = PM_FREEZE;
		// can exit intermission after five seconds
		if (level.time > level.intermissiontime + 15.0
			&& (client->buttons))// & BUTTON_ANY))
		{
			edict_t* player;
	...
	if (client->latched_buttons & BUTTON_ATTACK)
	{
		if (client->resp.spectator) {
			...
			//gi.bprintf(PRINT_HIGH, "DEBUG: FORWARDMOVE = %i, TIME = %f\n", client->ucmd->forwardmove, level.time);
			Think_Weapon(ent);
		}
	}
...
void ClientBeginServerFrame(edict_t* ent)
{
	...
	/*if (ent->client && level.time > 5)
	{
		//speed += (int)self->client->ucmd->forwardmove;
		gi.bprintf(PRINT_HIGH, "DEBUG: FORWARDMOVE = %i, TIME = %f\n", client->ucmd->forwardmove, level.time);
	...
	if (ent->deadflag)
	{
		//gi.bprintf(PRINT_HIGH, "DEAD: player is dead!, deadflag is set\n");
