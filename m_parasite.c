...
{
	gi.sound(self, CHAN_WEAPON, sound_tap, 0.4 + (random() * 0.2), ATTN_STATIC, 0);
}

void parasite_scratch (edict_t *self)
{
	gi.sound(self, CHAN_WEAPON, sound_scratch, 0.25 + (random() * 0.74), ATTN_STATIC, 0);
}
...
void SP_monster_parasite (edict_t *self)
{
	...
	self->monsterinfo.monster_type = MONSTER_PARASITE;
