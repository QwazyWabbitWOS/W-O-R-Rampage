...
//		skin = 0;
	...
	self->monsterinfo.currentmove = &insane_move_stand_normal;
	self->monsterinfo.monster_type = MONSTER_INSANE;
	...
	if (self->spawnflags & 8)					// Crucified ?
	{
		...
		self->movetype = MOVETYPE_NONE; //there is a bug that moves them at the start of the level, this fixes temporarily the problem
	}
	else
	{
