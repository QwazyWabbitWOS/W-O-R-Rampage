...
void BossExplode (edict_t *self)
{
	...
	{
	case 0:
		org[0] -= 24 * ( 0.75 + (crandom() * 0.5));
		org[1] -= 24 * (0.75 + (crandom() * 0.5));
		break;
	case 1:
		org[0] += 24 * (0.75 + (crandom() * 0.5));
		org[1] += 24 * (0.75 + (crandom() * 0.5));
	...
	case 3:
		...
		break;
	case 4:
		org[0] -= 48 * (0.75 + (crandom() * 0.5));
		org[1] -= 48 * (0.75 + (crandom() * 0.5));
		break;
	case 5:
		org[0] += 48 * (0.75 + (crandom() * 0.5));
		org[1] += 48 * (0.75 + (crandom() * 0.5));
...
void SP_monster_supertank (edict_t *self)
{
	...
	sound_firex = gi.soundindex("berserk/xfire.wav");
//	self->s.sound = gi.soundindex ("bosstank/btkengn1.wav");
	...
	self->svflags |= SVF_NOANGLECHASE;
	self->s.effects |= EF_FLAG1;
	self->monsterinfo.monster_type = MONSTER_SUPERTANK;
