...
mmove_t infantry_move_death4 = { FRAME_death401, FRAME_death406, infantry_frames_death4, infantry_dead };
...
void infantry_duckattack_down(edict_t* self)
{
	if (self->monsterinfo.aiflags & AI_DUCKED)
		return;
	self->monsterinfo.aiflags |= AI_DUCKED;
...
void infantry_duckattack_hold(edict_t* self)
{
	if (random() > 0.5 && self->groundentity)
	{
		self->monsterinfo.aiflags |= AI_JUMPDODGE;
...
void infantry_duckattack_up(edict_t* self)
{
	self->monsterinfo.aiflags &= ~AI_DUCKED;
	self->maxs[2] += 32;
	self->takedamage = DAMAGE_AIM;
...
void infantry_fire_duck(edict_t* self)
{
	InfantryMachineGun(self);

	if (!M_CheckClearShot(self))//|| level.time > self->monsterinfo.pausetime)
		self->monsterinfo.nextframe = FRAME_attak309;
	else if (self->s.frame == FRAME_attak307)
		self->monsterinfo.nextframe = FRAME_attak303;
}
mframe_t infantry_frames_attack3[] = //duck attack
{
	{ai_move, -2, infantry_duckattack_down},
	{ai_move, -5, NULL},
	{ai_move, 0,  infantry_fire_duck},
	{ai_move, 0,  infantry_fire_duck},
	{ai_move, 0,  infantry_fire_duck},
	{ai_move, 0,  infantry_fire_duck},
	{ai_move, 0,  infantry_fire_duck},
	{ai_move, 3,  NULL},
	{ai_move, 4,  infantry_duckattack_up},
	{ai_move, 0,  NULL}
};
mmove_t infantry_move_attack3 = { FRAME_attak301, FRAME_attak310, infantry_frames_attack3, infantry_run };
...
void InfantryMachineGun(edict_t* self)
{
	...
	if ((self->s.frame >= FRAME_attak111 && self->s.frame <= FRAME_attak116) ||
		(self->s.frame >= FRAME_attak302 && self->s.frame <= FRAME_attak308))
	{
		...
		if (self->s.frame >= FRAME_attak302 && self->s.frame <= FRAME_attak308)
			flash_number = MZ2_INFANTRY_MACHINEGUND_1;
		else
		...
		if (self->enemy)
		{
			...
			if (fire_rocket)
				predict_shot(self, start, 0, target, TYPE_PROJECTILE);
			else
				predict_shot(self, start, 0, target, TYPE_HITSCAN);
			VectorSubtract(target, start, forward);
			VectorNormalize(forward);
			//gi.bprintf(PRINT_HIGH, "DEBUG: Infantry has enemy, using prediction!\n");
		...
		{
			AngleVectors(self->s.angles, forward, right, NULL);
			//gi.bprintf(PRINT_HIGH, "DEBUG: Infantry has no enemy, using only angles!\n");
		}
	}
	else
	{
		...
		int length = sizeof(aimangles) / sizeof(aimangles[0]);
		int index = flash_number - MZ2_INFANTRY_MACHINEGUN_2;
		if (index > length - 1)
		{
			gi.bprintf(PRINT_HIGH, "DEBUG: Infantry aimangles out of bounds!, max = %i, was = %i\n", length, index);
			index = length - 1;
		}
		VectorSubtract(self->s.angles, aimangles[index], vec);
...
void SP_monster_infantry(edict_t* self)
{
	...
	self->monsterinfo.monster_type = MONSTER_INFANTRY;
