...
void plat_blocked (edict_t *self, edict_t *other)
{
	...
	{
	//	gi.bprintf(PRINT_HIGH, "plat_blocked: PLAT BLOCKED!CLIENT OR MONSTER! moveinfo.state = %i, other = %s, checking = %i \n", self->moveinfo.state, other->classname, checking);

		//G_FreeEdict(other);
		//return;
		// give it a chance to go away on it's own terms (like gibs)
		if (other->svflags & SVF_PROJECTILE)
			other->svflags |= SVF_NODIEDELAY;
		T_Damage (other, self, self, vec3_origin, other->s.origin, vec3_origin, 100000, 1, 0, MOD_CRUSH);
		// if it's still there, nuke it
		if (other->inuse)
			BecomeExplosion1 (other);
		if (other->inuse && !strcmp(other->classname, "bodyque")) //if the entity survived and it's a bodyqueue then make it not solid
	...
	//gi.bprintf(PRINT_HIGH, "plat_blocked: PLAT BLOCKED! moveinfo.state = %i, other = %s, checking = %i \n", self->moveinfo.state, other->classname, checking);
