...
============
*/
void bolt_ghosting(edict_t *ent, int mode)
{
	edict_t *e;
	int i;
	...
	{
		if (e->inuse && (e->svflags & SVF_BOLT) && e->owner_solid == ent->owner_solid && e != ent)
		{
			if (mode == MODE_GHOST)
			{
				VectorClear(e->mins);
			...
			{
				VectorSet(e->mins, -BOLT_SIZE, -BOLT_SIZE, -BOLT_SIZE);
				VectorSet(e->maxs, BOLT_SIZE, BOLT_SIZE, BOLT_SIZE);
				e->solid = SOLID_BBOX;
			}
		}
	}
...
retry:
	...
	if(!bolt_retry)
		trace = gi.trace (start, ent->mins, ent->maxs, end, ignore, mask);
	else
		trace = gi.trace(start, NULL, NULL, end, ignore, mask);
	if ((ent->svflags & SVF_BOLT) && (trace.ent->svflags & SVF_BOLT) )
	{
		if(ent->owner_solid != trace.ent->owner_solid) //touch other players bolts, but pass through them
			SV_Impact(ent, &trace);
		if (bolt_retry > 100)
		{
			//if (ent->svflags & FL_BOLT) // if there's is a better method for one owner bolts not touching themselves
			bolt_ghosting(ent, MODE_GHOST); // msg me //woju
		}
		//if(trace.ent->owner_solid != ent->owner_solid && trace.fraction != 1.0)
		//	SV_Impact(ent, &trace);
	...
	if(bolt_retry > 100)
		bolt_ghosting(ent, MODE_SOLID); //make all bolts of this owner solid back again
	VectorCopy (trace.endpos, ent->s.origin);

//	if (ent->svflags & FL_BOLT) 
//		
	//gi.bprintf(PRINT_HIGH, "BOLT: %i retries!", bolt_retry);
	gi.linkentity (ent);
	if ((ent->svflags & SVF_BOLT) && (trace.ent->svflags & SVF_BOLT) && trace.ent->owner_solid == ent->owner_solid && trace.fraction != 1.0)
	{
		//gi.bprintf(PRINT_HIGH, "BOLT: prevented from touching!");
			return trace;
	}
		
	else if (trace.fraction != 1.0)
	{
		if (ent->svflags & SVF_GRAVITYGRENADE && trace.ent->svflags & SVF_GRAVITYGRENADE)
		{
...
//retry:
	...
	if (pushed_p > &pushed[MAX_EDICTS])
		GameError("%s pushed_p > &pushed[MAX_EDICTS], memory corrupted", __func__);
