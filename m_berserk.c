...
void berserk_attack_club(edict_t *self)
{
	...
	if (skill->value > 3)
		fire_hit(self, aim, ((15 * skill->value) + (rand() % 6)), 800);		// Slower attack
	else
		fire_hit(self, aim, (5 + (rand() % 6)), 800);		// Slower attack
...
{
	static	vec3_t	aim = {MELEE_DISTANCE, 5, -24};
	if (skill->value > 3)
		fire_hit (self, aim, ((25 * skill->value) + (rand() % 6)), 400);		//	Faster attack -- upwards and backwards
	else
...
void berserk_duck_hold(edict_t *self)
{
	...
	else
	{
		...
		{
			self->monsterinfo.aiflags |= AI_HOLD_FRAME;
			if (random() < 0.25 && skill->value < 3) //funny duck jump with sneaky turning head left and right, like to see if someone is watching him
			{
